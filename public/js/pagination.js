...
const ul = document.querySelector('.post-list'); // 게시물을 담을 unordered list
...
const setPageOf = (pageNumber) => {
	...
	for (
		...
		postBoard.textContent = data[i - 1].postBoard;
    	postTitle.textContent = data[i - 1].title;
		postWriter.textContent = data[i - 1].postWriter;
		postDate.textContent = data[i - 1].postDate;
		postView.textContent = data[i - 1].postView;

    	postContainer.append(postNumber, postBoard, postTitle, postWriter, postDate, postView);
    	li.append(postContainer);
    	ul.append(li);
	}
};

// 제목에 hover 이펙트, 제목 부분 클릭시 해당 페이지 이동, 또는 반복문 안에 태그 반복?
